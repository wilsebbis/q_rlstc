"""
Legacy Split Method / Clustering Initializer (Baseline)
=====================================================

This file `rl_splitmethod.py` was used to initialize clustering centers using
DBSCAN, Agglomerative Clustering, and KMeans over the trajectory slices.

Mapping to `q_rlstc` Refactor:
------------------------------
Functionality from this script is partially ported and encapsulated within
`q_rlstc/data/rlstc_cluster.py` and the data preprocessing scripts.

Key Architectural Differences in the Refactor:
1. **Removed Dead Code**: Scikit-Learn DBSCAN, AgglomerativeClustering, and
   other imports that were rarely used in the main evaluation loops were pruned
   in favor of simpler baseline initialization during refactoring.
"""
