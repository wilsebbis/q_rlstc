"""
Legacy Trajectory Clustering Utilities (Baseline)
=================================================

This file `cluster.py` contains the original implementations of trajectory
distance calculations and online cluster updates.

Mapping to `q_rlstc` Refactor:
------------------------------
Refactored and ported to: `q_rlstc/data/rlstc_cluster.py`

Key Architectural Differences in the Refactor:
1. **Type Hints & Vectorization**: The original code relies exclusively on
   vanilla Python objects (`Point`, `Segment`) and nested while-loops. The new
   version includes type-hinted structures and removes some unreachable logical
   dead ends.
2. **Deterministic Tracking**: `update_centers` and distance computations were
   made cleaner and reproducible in the refactor.
"""
