"""
Legacy Training Loop Implementation (Baseline)
==============================================

This file `rl_train.py` contains the original training and evaluation loops
for the RLSTC codebase.

Mapping to `q_rlstc` Refactor:
------------------------------
This file has been fully deprecated and completely replaced by the modern
experiment runners located in `experiments/`:
- `experiments/run_cross_comparison.py`: The definitive test harness for 
  evaluating models side-by-side.
- `experiments/run_all.sh`: The suite orchestrator.

Key Architectural Differences in the Refactor:
1. **CMDP Support**: The refactored training loops integrate Constrained MDP
   (CMDP) Lagrange multiplier tracking directly into the evaluation epochs.
2. **Modular Checkpointing**: Model saving and performance reporting were
   made far more modular, and the refactor tracks proper evaluation 
   metrics (`cut_pct`, `empirical_cut_rate`, etc.).
"""
