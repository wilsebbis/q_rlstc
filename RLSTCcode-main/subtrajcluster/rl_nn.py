"""
Legacy Deep Q-Network Implementation (Baseline)
===============================================

This file `rl_nn.py` contains the original classical DQN implementation 
using Keras with a TensorFlow 1.x backend (`tf.disable_v2_behavior()`).

Mapping to `q_rlstc` Refactor:
------------------------------
This file has been fully deprecated. The Q-RLSTC refactor contains two 
modern replacements:
1. `q_rlstc/rl/classical_agent.py`: A native PyTorch implementation of the 
   classical DQN, structurally matched to the quantum version.
2. `q_rlstc/rl/vqdqn_agent.py`: The Hybrid Quantum-Classical VQ-DQN agent,
   using SPSA gradients and pure numpy/Qiskit execution.

Key Architectural Differences in the Refactor:
1. **Modern Stack**: Removed TF1 compatibility layers in favor of pure 
   PyTorch (Classical) and Qiskit/Numpy SPSA (Quantum).
2. **Replay Buffer Extraction**: The internal list-based `deque` was 
   extracted into `q_rlstc/rl/replay_buffer.py` allowing fast Numpy
   vectorized sampling.
3. **Double DQN Setup**: The new agents include True Double DQN with
   polyak averaging, missing in this simple DQN implementation.
"""
