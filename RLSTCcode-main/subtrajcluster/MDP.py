"""
Legacy Trajectory RL Clustering MDP Implementation (Baseline)
=============================================================

This file (`MDP.py`) contains the original implementation of `TrajRLclus`, 
the core environment simulating the sequential trajectory segmentation process.

Mapping to `q_rlstc` Refactor:
------------------------------
This file has been completely refactored and ported to:
`q_rlstc/data/rlstc_mdp.py`

Key Architectural Differences in the Refactor:
1. **Separation of Concerns**: In this legacy file, tracking evaluation metrics,
   updating the clustering dictionary, and computing episode rewards are heavily
   entangled.
2. **Fixed Metric Pathologies**: The rewards and states calculated here originally
   suffered from ValCR fragmentation degeneracy (excessive segmentation). The new 
   refactor introduces Constrained MDP (CMDP) Lagrangian multipliers.
3. **Observation Scaling**: This baseline returns raw unscaled observations 
   (`self.overall_sim`, `self.minsim`), leading to training instability. The 
   refactored version uses `Z-scaling` via `ObservationTracker`.
"""
